    ...
    <template>
      ...
      <router-link v-if="hasOneShowingChildren(item.children, item) && (!onlyOneChild.children || onlyOneChild.noShowingChild) && !item.alwaysShow" :to="resolvePath(onlyOneChild.path)">
        <el-menu-item :index="item.path">
          <item :icon="item.children[0].meta.icon" :title="$t(item.children[0].meta.title)"></item>
        </el-menu-item>
      </router-link>

      <el-submenu v-else :index="item.path">
        <template slot="title">
          <item :icon="item.meta.icon" :title="$t(item.meta.title)"></item>
        </template>
        ...
        <template v-for="child of item.children" v-if="!child.hidden">
          ...
          <router-link v-else :to="resolvePath(child.path)">
            <el-menu-item :index="child.path">
              <item :icon="child.meta.icon" :title="$t(child.meta.title)"></item>
            </el-menu-item>
          </router-link>
        </template>
      </el-submenu>
